import Link from 'next/link'
import { MapPin, Phone, Mail, Clock, Facebook, Cog } from 'lucide-react'
import { COMPANY, WHATSAPP_URL, EMAIL_MAILTO } from '@/lib/constants'

export default function Footer() {
  return (
    <footer className="bg-brand-gray-dark border-t border-brand-gray">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-10">
          {/* Col 1: Brand */}
          <div>
            <div className="flex items-center gap-2 mb-4">
              <div className="flex items-center justify-center w-9 h-9 bg-brand-orange rounded-md">
                <Cog className="w-5 h-5 text-white" />
              </div>
              <div className="flex flex-col leading-none">
                <span className="text-brand-orange font-bold text-base tracking-wide">AP</span>
                <span className="text-white text-[10px] tracking-widest uppercase">Pisanelli</span>
              </div>
            </div>
            <p className="text-gray-400 text-sm leading-relaxed mb-4">
              Impianto autorizzato con {COMPANY.yearsExperience} anni di esperienza.
              Rottamazione con ritiro gratuito, cancellazione PRA e ricambi usati garantiti.
              Risposta entro 30 minuti in orario lavorativo.
            </p>
            <p className="text-gray-500 text-xs">
              P.IVA {COMPANY.vatNumber} · Cod. SDI {COMPANY.sdiCode}
            </p>
          </div>

          {/* Col 2: Navigazione + Orari */}
          <div>
            <h3 className="text-white font-semibold mb-4 text-sm uppercase tracking-wider">
              Pagine
            </h3>
            <nav className="space-y-2 mb-6">
              {[
                { href: '/', label: 'Home' },
                { href: '/rottamazione', label: 'Rottamazione' },
                { href: '/servizi', label: 'Servizi' },
                { href: '/ricambi', label: 'Ricambi' },
                { href: '/faq', label: 'FAQ' },
                { href: '/contatti', label: 'Contatti' },
              ].map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  className="block text-gray-400 hover:text-brand-orange text-sm transition-colors"
                >
                  {link.label}
                </Link>
              ))}
            </nav>

            <h3 className="text-white font-semibold mb-3 text-sm uppercase tracking-wider flex items-center gap-2">
              <Clock className="w-4 h-4 text-brand-orange" />
              Orari
            </h3>
            <div className="space-y-1 text-sm">
              {COMPANY.hours.map((h) => (
                <div key={h.days} className="flex justify-between text-gray-400">
                  <span>{h.days}</span>
                  <span>
                    {h.morning
                      ? h.afternoon
                        ? `${h.morning} / ${h.afternoon}`
                        : h.morning
                      : 'Chiuso'}
                  </span>
                </div>
              ))}
            </div>
          </div>

          {/* Col 3: Contatti */}
          <div>
            <h3 className="text-white font-semibold mb-4 text-sm uppercase tracking-wider">
              Contatti
            </h3>
            <div className="space-y-3 text-sm">
              <a
                href={`https://maps.google.com/?q=${encodeURIComponent(COMPANY.address.full)}`}
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-start gap-2 text-gray-400 hover:text-brand-orange transition-colors"
              >
                <MapPin className="w-4 h-4 mt-0.5 flex-shrink-0 text-brand-orange" />
                <span>{COMPANY.address.full}</span>
              </a>

              <a
                href={`tel:${COMPANY.contacts.phone.tiziano.tel}`}
                className="flex items-center gap-2 text-gray-400 hover:text-brand-orange transition-colors"
              >
                <Phone className="w-4 h-4 flex-shrink-0 text-brand-orange" />
                <span>{COMPANY.contacts.phone.tiziano.number} (Tiziano)</span>
              </a>

              <a
                href={EMAIL_MAILTO}
                className="flex items-center gap-2 text-gray-400 hover:text-brand-orange transition-colors"
              >
                <Mail className="w-4 h-4 flex-shrink-0 text-brand-orange" />
                <span className="break-all">{COMPANY.contacts.email}</span>
              </a>

              {/* Social */}
              <div className="flex items-center gap-3 pt-2">
                <span
                  className="flex items-center gap-1.5 text-gray-600 cursor-not-allowed text-xs"
                  title="Pagina Facebook in arrivo"
                >
                  <Facebook className="w-4 h-4" />
                  <span>Facebook (In arrivo)</span>
                </span>
              </div>

              <a
                href={WHATSAPP_URL}
                target="_blank"
                rel="noopener noreferrer"
                className="inline-flex items-center gap-2 mt-2 px-4 py-2 bg-[#25D366] hover:bg-[#1ebe57] text-white text-sm font-medium rounded-md transition-colors"
              >
                <svg width="16" height="16" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
                  <path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z" />
                </svg>
                WhatsApp
              </a>
            </div>
          </div>
        </div>
      </div>

      {/* Copyright bar */}
      <div className="border-t border-brand-gray">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex flex-col sm:flex-row items-center justify-between gap-2 text-xs text-gray-500">
          <span>© {new Date().getFullYear()} {COMPANY.name}. Tutti i diritti riservati.</span>
          <span>P.IVA {COMPANY.vatNumber}</span>
        </div>
      </div>
    </footer>
  )
}
